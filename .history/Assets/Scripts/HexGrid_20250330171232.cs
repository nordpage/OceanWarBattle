...
public class HexGrid : MonoBehaviour
{
    ...
    private Dictionary<Vector3Int, Hex> hexGrid = new Dictionary<Vector3Int, Hex>();
    ...
    Vector3 CalculateHexPosition(int x, int y, int z)
    {
    ...
    private int GetMovementCost(Vector3Int position, Ship ship)
    {
    ...
    public void MoveShip(Ship ship, Vector3Int targetPos)
    {
        ...
        ship.HexPosition = targetPos;
        if (hexGrid.ContainsKey(targetPos))
            hexGrid[targetPos].OccupyingShip = ship;
        
        // Обновляем мировую позицию
        Vector3 worldPos = CalculateHexPosition(targetPos.x, targetPos.y, targetPos.z);
        worldPos.y = 0.3f;  // Поднимаем немного над сеткой
        
        // Плавное движение
        StartCoroutine(MoveShipSmoothly(ship, worldPos));
        
        // Уменьшаем очки движения
        int moveCost = GetMovementCost(targetPos, ship);
        ship.MovementPoints -= moveCost;
        ...
        if (ship.OnMoved != null)
            ship.OnMoved(ship, oldPos, targetPos);
    }
    ...
    public void HandleHexClick(Vector3Int hexPos)
    {
        ...
        if (!hexGrid.ContainsKey(hexPos))
            return;
        
        Hex clickedHex = hexGrid[hexPos];
        Ship shipAtPos = clickedHex.OccupyingShip;
        
        // Если выбран корабль игрока
        if (shipAtPos != null && shipAtPos.PlayerOwned)
        {
            SelectShip(shipAtPos);
            return;
        }
        
        // Если уже выбран корабль для действия
        if (SelectedShip != null)
        {
            ...
            // Атака
            if (shipAtPos != null && !shipAtPos.PlayerOwned && IsInAttackRange(SelectedShip, shipAtPos))
            {
                SelectedShip.Attack(shipAtPos);
                ClearHighlights();
                
                // Проверяем, есть ли у нас корабли с доступными действиями
                bool allSpent = true;
                foreach (Ship ship in GameManager.Instance.PlayerShips)
                {
                    if (ship.MovementPoints > 0 || ship.CanAttack)
                    {
                        allSpent = false;
                        break;
                    }
                ...
                // Если все корабли выполнили действия, завершаем ход
                if (allSpent)
                {
