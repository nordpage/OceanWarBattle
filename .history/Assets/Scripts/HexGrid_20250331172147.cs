...
public void HandleHexClick(Vector3Int hexPos)
{
    ...
    // Получаем UI для действий корабля
    ...
    // Если есть выбранный корабль и режим действия
    if (selectedShip != null && actionUI != null && actionUI.gameObject.activeSelf)
    {
        ShipActionUI.ActionMode mode = actionUI.GetCurrentMode();
        
        switch (mode)
        {
            case ShipActionUI.ActionMode.Move:
                HandleMoveAction(selectedShip, hexPos);
                break;
                
            case ShipActionUI.ActionMode.Rotate:
                HandleRotateAction(selectedShip, hexPos);
                break;
                
            case ShipActionUI.ActionMode.Fire:
                HandleFireAction(selectedShip, hexPos);
                break;
                
            case ShipActionUI.ActionMode.None:
                // Если режим не выбран, но есть выбранный корабль,
                // просто показываем UI для этого корабля
                break;
        }
    ...
    // Если нет выбранного корабля, проверяем, есть ли на клетке корабль игрока
    Hex hex = null;
    if (hexGrid.TryGetValue(hexPos, out hex) && hex.OccupyingShip != null)
    {
        Ship ship = hex.OccupyingShip;
        
        if (ship.PlayerOwned && GameManager.Instance.IsPlayerTurn)
        {
            // Выбираем корабль
            ship.Select();
            
            // Показываем UI действий для этого корабля
            if (actionUI != null)
            {
                actionUI.ShowForShip(ship);
            }
        }
    }
}
...
{
    // Преобразуем в кубические координаты
    Vector3 fromCube = AxialToCube(new Vector2(from.x, from.z));
    Vector3 toCube = AxialToCube(new Vector2(to.x, to.z));
    
    // Вычисляем вектор направления
    Vector3 direction = toCube - fromCube;
    
    // Нормализуем до единичного шага (берем самое большое значение из x,y,z)
    float maxComponent = Mathf.Max(Mathf.Abs(direction.x), Mathf.Abs(direction.y), Mathf.Abs(direction.z));
    if (maxComponent > 0)
    {
        direction /= maxComponent;
    }
    
    // Округляем до ближайшего из шести направлений
    ...
    // Преобразуем кубические направления в индекс направления (0-5)
    if (direction.x == 1 && direction.y == -1 && direction.z == 0) return 0; // Восток
    if (direction.x == 0 && direction.y == -1 && direction.z == 1) return 1; // Северо-восток
    if (direction.x == -1 && direction.y == 0 && direction.z == 1) return 2; // Северо-запад
    if (direction.x == -1 && direction.y == 1 && direction.z == 0) return 3; // Запад
    if (direction.x == 0 && direction.y == 1 && direction.z == -1) return 4; // Юго-запад
    if (direction.x == 1 && direction.y == 0 && direction.z == -1) return 5; // Юго-восток
    
    // По умолчанию сохраняем текущее направление корабля
    return 0;
}

// Обновленный метод MoveShip
