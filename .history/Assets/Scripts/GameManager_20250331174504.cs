...
public class GameManager : MonoBehaviour
{
 ...
 // Изменить метод Start в GameManager
    private void Start()
    {
        // Инициализация игры
        ...
        // Скрыть панель победы сразу при старте
        ...
        // Начало игры (НЕ проверяем условия победы при первом запуске)
        IsPlayerTurn = true;
        UpdateWind();
        UpdateUI();
    }
  ...
  private void PlaceShip(Ship ship, Vector3Int hexPosition)
    {
        Debug.Log($"Размещаем корабль {ship.name} на позиции {hexPosition}");
        
        // Устанавливаем корабль в сетке
        if (hexGrid.hexGrid.ContainsKey(hexPosition))
            hexGrid.hexGrid[hexPosition].OccupyingShip = ship;
        
        // Определяем начальное направление
        int initialDirection = 0; // По умолчанию восток
        if (!ship.PlayerOwned)
            initialDirection = 3; // Запад для кораблей противника
        
        // Используем MoveToPosition для установки позиции с указанием направления
        ship.MoveToPosition(hexPosition, initialDirection);
    }
    ...
    // Добавить в класс GameManager
public void CreateExplosion(Vector3Int position)
{
    Vector3 worldPos = hexGrid.CalculateHexPosition(position.x, position.y, position.z);
    worldPos.y = 0.5f; // Чуть выше над поверхностью воды
    
    // Создаем объект эффекта
    GameObject explosionObject = new GameObject("Explosion");
    explosionObject.transform.position = worldPos;
    
    // Добавляем систему частиц
    ParticleSystem ps = explosionObject.AddComponent<ParticleSystem>();
    var main = ps.main;
    main.startColor = new Color(1f, 0.5f, 0f, 1f); // Оранжевый цвет
    main.startSize = 1.5f;
    main.startLifetime = 1f;
    main.duration = 0.5f;
    main.loop = false;
    
    // Автоматически уничтожаем объект через 2 секунды
    Destroy(explosionObject, 2f);
}

public void CreateSplash(Vector3Int position)
{
    Vector3 worldPos = hexGrid.CalculateHexPosition(position.x, position.y, position.z);
    worldPos.y = 0.1f; // Чуть выше поверхности воды
    
    // Создаем объект эффекта
    GameObject splashObject = new GameObject("Splash");
    splashObject.transform.position = worldPos;
    
    // Добавляем систему частиц
    ParticleSystem ps = splashObject.AddComponent<ParticleSystem>();
    var main = ps.main;
    main.startColor = new Color(0.7f, 0.7f, 1f, 0.8f); // Голубоватый цвет
    main.startSize = 1f;
    main.startLifetime = 0.8f;
    main.duration = 0.3f;
    main.loop = false;
    
    // Автоматически уничтожаем объект через 1 секунду
    Destroy(splashObject, 1f);
}
...
private void MoveTowardsTarget(Ship ship, Ship target)
{
    ...
    while (ship.MovementPoints > 0)
    {
        ...
        if (hexGrid.CanMoveTo(nextPosition, ship) && !hexGrid.IsOccupied(nextPosition))
        {
            ...
            int newDirection = hexGrid.CalculateDirectionTowards(ship.HexPosition, nextPosition);
            
            // Перемещаем корабль
            hexGrid.MoveShip(ship, nextPosition, newDirection);
