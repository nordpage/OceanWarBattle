...
public class Ship : MonoBehaviour
{
    ...
    public Image HealthBarImage; // Заменяем Slider на Image

    [Header("Materials")]
    public Material PlayerMaterial;
    public Material EnemyMaterial;
    public Material SelectionMaterial; // Материал для выделения корабля

    // Храним оригинальный материал для восстановления
    ...
    public void UpdateVisuals()
    {
        if (ShipMesh != null)
        {
            // Устанавливаем базовый материал в зависимости от владельца
            originalMaterial = PlayerOwned ? PlayerMaterial : EnemyMaterial;
            
            // Если выделен, используем материал выделения, иначе - базовый
            ShipMesh.material = isSelected ? SelectionMaterial : originalMaterial;
        }
    ...
    public void UpdateHealthBar()
    {
        if (HealthBarImage != null)
        {
            // Устанавливаем fillAmount вместо value для Slider
    ...
    public void Select()
    {
        ...
        if (ShipMesh != null && SelectionMaterial != null)
            ShipMesh.material = SelectionMaterial;
    ...
    public void Deselect()
    {
        ...
        if (ShipMesh != null)
            ShipMesh.material = originalMaterial;
    }
    ...
    private System.Collections.IEnumerator DamageFlashEffect()
    {
        if (ShipMesh != null)
        {
            // Сохраняем текущий материал (может быть или обычный, или выделения)
            Material currentMaterial = ShipMesh.material;
            ShipMesh.material.color = Color.red;
            yield return new WaitForSeconds(0.1f);
            ShipMesh.material = currentMaterial;
        }
    }
